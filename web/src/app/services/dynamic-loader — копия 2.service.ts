...
export class DynamicLoaderService {
  ...
  /**
   * Load a standalone component from remote (existing function)
   */
  async loadStandaloneComponent<T>(remoteEntry: string, exposedModule: string, remoteName: string): Promise<T> {
    try {
      console.log('Loading remote component:', { remoteEntry, remoteName, exposedModule });
  ...
  /**
   * Load a module from remote (new function - same implementation but different return type)
   */
  ...
  /**
   * Load remote entry script (shared by both functions)
   */
